...
def run_detection():
    ...
    character_detections = {
        ...
        'unknown': {'detections': [], 'scores': [], 'file_names': []}  # Added unknown
    }
    ...
    # Save task2 arrays with proper formatting
    for char in character_detections:
        if character_detections[char]['detections']:
            # Convert lists to properly formatted numpy arrays
            ...
            # Ensure detections are properly shaped
            if len(detections.shape) == 1:
                detections = detections.reshape(-1, 4)
            
            # Save arrays with proper format
